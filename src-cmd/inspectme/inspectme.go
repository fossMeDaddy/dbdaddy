...
func run(cmd *cobra.Command, args []string) {
	...
	dbTables, err := db_int.ListTablesInDb(viper.GetString(constants.DbConfigCurrentBranchKey))
	if err != nil {
		cmd.PrintErrf("Unexpected error occured while fetching tables from database '%s'\n%s", currBranch, err.Error())
		return
	}
	dbStrTables := []string{}
	...
	for _, tablename := range selectedTables {
		...
		if err != nil {
			cmd.PrintErrf("Unexpected error occured while fetching table schema for %s\n"+err.Error(), tablename)
			return
		}
